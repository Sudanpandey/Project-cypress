...
describe("Inspection Templates", () => {
    it("Should display error message for empty template name", () => {
        cy.login();
        // Navigation
        cy.get(':nth-child(11) > .nav-item > .icon-sidenav',{timeout: 20000}).click();
    ...
    it("Should be listed at the top of the page after creating new contracts", () => {
        ...
        cy.get('.card-footer > .btn-secondary').click();
        cy.get('.notification-content').should('contain','The template was successfully added'); 
    ...
    it.only("Should redirect to the inspection template page when clicking the back button", () => {
        cy.login();
        // Navigation
        cy.get(':nth-child(11) > .nav-item > .icon-sidenav',{timeout: 20000}).click();
        cy.get('.nav > :nth-child(4) > .nav-link').click();
