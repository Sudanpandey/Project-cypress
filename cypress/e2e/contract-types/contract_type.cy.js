...
describe("Create Contract-type", () => {
    it("Should display error message for empty contracts-type", () => {
        ...
        cy.get('.text-danger.order-5').should('contain', 'Het name veld is verplicht.');
    ...
    it("Should create the new contract-type after entering name and click save button ", () => {
        ...
        cy.get('button[class="btn btn-primary"]').click();
        cy.get('.notification-content').should('contain','Contracttype is succesvol opgeslagen.');
        cy.get('tbody>tr:nth-child(1)').should('contain',contractTypeName);
        //Search recently created item
        cy.get("input[placeholder='Search']").clear().type(contractTypeName).type('{enter}');
        cy.get('tbody>tr:nth-child(1)').should('contain',contractTypeName);
        ...
        cy.contains('Contracttype is succesvol opgeslagen.').should('be.visible').click();
        cy.wait(5000);
    })
    it("Should open a edit modal when edit button is click", () => {
        ...
        //Search recently created item
        cy.get("input[placeholder='Search']").clear().type(editContractTypeName).type('{enter}');
        cy.get('tbody>tr:nth-child(1)').should('contain',editContractTypeName);
        cy.get('tbody').find('tr').its('length').should('be.eq', 1);
        ...
        cy.get("input[name='name']").clear().type('Edited'+ editContractTypeName);
        cy.get('button[class="btn btn-primary"]').click();
        cy.contains('Contracttype is succesvol opgeslagen.').should('be.visible').click();
        cy.contains('success').click();
        //Search recently created item
        cy.get("input[placeholder='Search']").clear().type('Edited'+ editContractTypeName).type('{enter}');
        cy.get('tbody>tr:nth-child(1)').should('contain','Edited'+ editContractTypeName);
        cy.get('tbody').find('tr').its('length').should('be.eq', 1);
        //Delete recently created contract-type
        cy.get('.mdi.mdi-cog').click();
        cy.contains('Verwijderen').click();
    ...
    it("should be empty validation in the edit action", () => {
        ...
        cy.get('button[class="btn btn-primary"]').click();
        cy.get('.notification-content').should('contain','Contracttype is succesvol opgeslagen.');
        cy.get('tbody>tr:nth-child(1)').should('contain',emptyEditContractTypeName);
        ...
        cy.get('button[class="btn btn-icon btn-light"]').click();
        //Search recently created item
        cy.get("input[placeholder='Search']").clear().type(emptyEditContractTypeName).type('{enter}');
        cy.get('tbody>tr:nth-child(1)').should('contain',emptyEditContractTypeName);
        cy.get('tbody').find('tr').its('length').should('be.eq', 1);
        ////Click edit button for recently created item
        cy.get('.mdi.mdi-cog').click();
